...
-   For multiple linear regression (MLR) model we use Python scikit -learn package.
...
Ridge regression is a variant of MLR designed to fit a linear model to the dataset by minimizing the following regularized least-square loss function $$L_{ridge}(y,f(X,w)) = \sum_{i=1}^N||y_i - X_iw - w_0||^2 + \alpha[||w||^2 + w_0^2]$$,

where $$\alpha$$ is the hyperparameter for ridge regression. Note that the ridge regression model reduces to MLR when �=0. By increasing the value of �, we can control the complexity of the model as will be shown in the example below.
