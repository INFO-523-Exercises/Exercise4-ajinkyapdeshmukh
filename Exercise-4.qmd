...
-   For multiple linear regression (MLR) model we use Python scikit -learn package. Given a training set $$X, y$$ MLR is designed to learn the regression function $$f(X,w) = X^Tw + w_0$$ by minimizing the following loss function given a training set $$(X_i,y_i)_{i=1}^N$$:
